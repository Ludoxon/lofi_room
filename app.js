(function() {
  ...
  var AudioManager = {
    ...
    init: function() {
      ELEMENTS.forEach(function(el) {
        if (el.audioId) {
          AudioManager.audioElements[el.id] = document.getElementById(el.audioId);
        }
      });
      AudioManager.audioElements['pageflip'] = document.getElementById('audio-pageflip');
    },

    play: function(elementId) {
      var audio = this.audioElements[elementId];
      if (audio) {
        audio.volume = state.elements[elementId].volume / 100;
        audio.currentTime = 0;
    ...
    playOneShot: function(soundId, volumeRef) {
      var audio = this.audioElements[soundId];
      if (audio) {
